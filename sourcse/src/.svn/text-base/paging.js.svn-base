	...
	Paging.prototype = {
		...
		bindEvent: function() {
			...
			this.container.on('click', 'li.js-page-action,li.ui-pager', function(e) {
				if ($(this).hasClass('ui-pager')) {
						for (var i=0;i<$('.onestore').length;i++) {
							$('.onestore:eq('+i+')').css({'display':'none'});
						}
						$('.onestore:eq('+($(this)[0].innerHTML*3-1)+')').css({'display':'block'});
						$('.onestore:eq('+($(this)[0].innerHTML*3-2)+')').css({'display':'block'});
						$('.onestore:eq('+($(this)[0].innerHTML*3-3)+')').css({'display':'block'});
				}
				...
				if ($(this).hasClass('js-page-action')) {
					...
					if ($(this).hasClass('js-page-prev')) {
						var current = Math.max(1, _this.current - 1);
					...
					for (var i=0;i<$('.onestore').length;i++) {
						...
						for (var i=current*3-3;i<current*3;i++) {
							$('.onestore:eq('+i+')').css({'display':'block'});
						}
